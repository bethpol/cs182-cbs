...
def generate_config_file(
    ...
    # Max tokens: just the window size (tokens_seen resets to 0 for branches)
    # The dataloader starts at checkpoint_tokens + (seed * window), but tokens_seen starts at 0
    # So we train for BRANCH_WINDOW_SIZE_TOKENS new tokens
    checkpoint_tokens = parse_checkpoint_tokens(checkpoint_step)
    max_tokens = BRANCH_WINDOW_SIZE_TOKENS
